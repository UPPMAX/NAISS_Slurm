...
For Dardel we provide two build scripts to generate executables for the CRAY Clang compiler and the GNU compiler respectively.  On the commandline, type
...
When using the CRAY compiler, hyperthreading can be disengaged by choosing a **cores** binding using the environment variable **OMP_PLACES**.   When using a chosing a **threads** binding hyperthreading will be used.
